...
// Railway может оказаться в регионе, где основной api.binance.com отвечает 451.
// Все адреса ниже — официальные публичные узлы Binance с одинаковым Spot API.
const BASES = [
  ...
  for (const base of BASES) {
    try {
      const r = await fetch(base + path, {
        headers: { 'User-Agent': 'IT-v3' },
        signal: AbortSignal.timeout(8_000),
      })
      if (r.ok) return r.json()
      errors.push(`${new URL(base).host} ${r.status}: ${(await r.text()).slice(0, 80)}`)
    } catch (err) {
      errors.push(`${new URL(base).host}: ${err.message}`)
    }
  }
  throw new Error(`binance unavailable (${errors.join('; ')})`)
}
