...
// График и алерты — часть PRO. Прямая ссылка подписана сервером,
// а Mini App передаёт проверяемый Telegram initData.
async function proUser(req, res) {
...
router.get('/symbols', async (req, res) => {
  ...
  try {
    if (!await proUser(req, res)) return
...
router.get('/alerts', async (req, res) => {
  try {
    const id = await proUser(req, res)
    if (!id) return
