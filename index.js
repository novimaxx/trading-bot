...
app.get('/api/signals', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT pair, action, tag, timeframe, price, comment, direction, sent_at FROM app_signals WHERE (tag NOT IN ('Тренд вверх','Тренд вниз') OR timeframe = '1D') ORDER BY sent_at DESC LIMIT 30`
    )
